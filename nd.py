...
def run(cfg: ModulusConfig) -> None:
    ...
    fluid_density  = quantity(997, "kg/m^3") #fluid_density
    fluid_dynamic_viscosity = quantity(0.0008899, "kg/(m*s)") #dynamic viscosity
    fluid_kinematic_viscosity = fluid_dynamic_viscosity/fluid_density #m^2/s

    Um = Re*fluid_kinematic_viscosity/D1
    
    outlet_p = quantity(0.0, "pascal")
    ...
    # fluid params
    nd_fluid_kinematic_viscosity = channelNonDim.ndim(fluid_kinematic_viscosity)
    nd_fluid_density = channelNonDim.ndim(fluid_density)

    # boundary params
    nd_inlet_velocity = channelNonDim.ndim(Um)
    nd_outlet_p = channelNonDim.ndim(outlet_p)


    # make list of nodes to unroll graph on
    ns = NavierStokes(nu=nd_fluid_kinematic_viscosity, nd_fluid_density=fluid_density, dim=2, time=False)
    ...
    velprof = nd_inlet_velocity*2*(1-(Abs(y)/(D1/2))**2)
